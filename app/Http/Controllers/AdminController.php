...
class AdminController extends Controller
{
    ...
    public function index(){
        ...
        // Data kunjungan website per hari
        $websiteVisitsByDay = WebsiteVisit::select('visit_date', 'visits')
            ->orderBy('visit_date')
            ->get();

        // $productData = DB::table('produk')
        //                 ->select('nama_produk', 'visitors')
        //                 ->get();    

        // Data kunjungan produk per hari
        $productVisitsByDay = ProductVisit::select('visit_date', 'visits')
            ->orderBy('visit_date')
            ->get();

        
        return view('contents.admin.home', compact('data', 'websiteVisitsByDay', 'productVisitsByDay'));
    }
    ...
    public function create_produk(Request $request)
    {
        ...
        if($request->file('gambar')){
           $validatedData['gambar'] = $request->file('gambar')->store('post-produk');

            // $image->move('./img_produk', $request->file('gambar')->getClientOriginalExtension());
            // $validator->gambar = $request->file('gambar')->getClientOriginalExtension();
            ...
            return redirect()->back()->withErrors($validatedData)->with("failed"," Gagal Insert Data ! ");
        }
    }
    ...
    public function update_produk(Request $request)
    {
        ...
        if($validator->passes()){
            ...
            if($request->file('gambar')){
                ...
                if($validator->passes()){
                    $validator['gambar'] = $request->file('gambar')->store('post-produk');
                }
                else{
